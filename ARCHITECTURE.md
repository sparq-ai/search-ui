...
**Second wire seam.** `insights/insightsClient.ts` is the tracking-API counterpart of `sparqClient.ts`: the only place that knows the st-tracking REST contract (endpoint, auth headers, `uId` identity cookie), kept byte-compatible with the legacy `@sparq/analytics-js` SDK so server-side processing and visitor identity are continuous. Payload field names that reuse legacy wire vocabulary live in `insights/insights.ts`; both files' wire knowledge is deliberate and allowlisted where lint applies (§19). Transport is `fetch keepalive` (events fire as the page navigates away) with a `sendBeacon` query-param fallback. Every storage or network touch swallows failures — analytics must never break search or checkout.
