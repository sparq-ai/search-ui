...
## 20. Insights (search analytics)

Opt-in analytics that make CTR, search conversion and search-attributed revenue measurable. Enabled by the `insights` attribute on `<sparq-search>` (or `configureInsights()` / `window.sparq('init', …)` on pages without a search element, e.g. order confirmation).

**Attribution model — queryID chaining (the Algolia pattern).** Every search response carries a server-assigned `queryId`. The insights middleware subscribes to the controller bus: a settled search emits `search-query` (plus `empty-search-query` at zero results, and `search-session` once per 30-minute activity window); a result click emits `product-clicked` carrying the queryId *of the page that produced the clicked item* (a parallel per-item list in the controller keeps infinite-scroll clicks attributed to their own page's search) and records `{itemId → queryId}` in a capped, 1-hour-windowed localStorage click map. `window.sparq('purchase', {orderId, items})` on the confirmation page rejoins each line item to its search through that map. Counting rule: every backend request with a fresh queryId is one tracked search (loadMore pages included); the queryId dedupe only suppresses replays of the same request.

**The engine is the SDK.** The analytics engine — wire client for the st-tracking REST contract (endpoint, auth headers, `uId` identity cookie), click map, session windowing, queryId dedupe, purchase rejoin — lives in the external `@sparq/analytics-js` (v2), which this library consumes as a dependency. That package is the canonical implementation for every Sparq surface (widget, custom integrations, platform modules), so attribution rules can never drift between them. What remains in this repo (`insights/insights.ts`) is only the search-ui-specific middleware: mapping controller bus events onto SDK calls, plus a facade that keeps the SDK's singleton and the provider lifecycle in step. Transport is `fetch keepalive` (events fire as the page navigates away) with a `sendBeacon` query-param fallback; every storage or network touch swallows failures — analytics must never break search or checkout. Wire vocabulary (including legacy field names, §19) is entirely the SDK's concern.
